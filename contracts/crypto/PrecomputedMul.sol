...
// Local derivative of SCL d714e9824e2e2e44be3c8fd498e0de651ddc9425.
// Changes: scratch allocation is relative to the actual free-memory pointer;
// the final ModExp buffer is reserved rather than resetting to an absolute address.
// Also returns affine Y and rejects degenerate precomputation tables. Unaudited.



import {_ModExpError} from "../../vendor/scl/src/include/SCL_errcodes.sol";
...
function ecPrecomputedMulAdd(
       ...
       // bytes memory Mem = new bytes(16*4*32);
        assembly ("memory-safe") {
         ...
         // Scratch starts at the current free-memory pointer; reserve it below.
        ...
        // Fail closed on exceptional keys whose precomputation table contains
        // an infinity/degenerate point. Preparation exercises this same check.
        for { let i := 1 } lt(i, 16) { i := add(i, 1) } {
            if iszero(mload(add(mload(0x40), add(shl(7, i), 64)))) { revert(0, 0) }
        }
        /*II. First MSB bit*/
                ZZZ:=0
             ...
             // let T1:=add(add(sub(1,iszero(and(scalar_u, mask))), shl(1,sub(1,iszero(and(scalar_u, T4))))),
              ...
              //inlined ec_Add
               ...
               if iszero(ZZ) {
                ...
                Y := mulmod(Y, mload(T), _p)
                ZZ := mulmod(ZZ, mload(T), _p)
                ZZ := mulmod(ZZ, ZZ, _p)
                X := mulmod(X, ZZ, _p) // normalize both coordinates   
